...
function LabSection() {
  ...
  const counts = {
    all: labEntries.length,
    shipped: labEntries.filter(e => e.status === 'shipped').length,
    'in-progress': labEntries.filter(e => e.status === 'in-progress').length,
    stuck: labEntries.filter(e => e.status === 'stuck').length,
  };

  return (
    <section id="lab" className="section lab-section">
      <div className="container">
        <div className="section-header reveal lab-header">
          <p className="section-label">// the_process</p>
          <h2 className="section-heading">Builder&apos;s Lab</h2>
          <p className="lab-intro">
            A live log of how each project actually got built — the ideas that worked, the
            experiments that didn&apos;t, and what I&apos;d do differently. Proof of work, not just the highlight reel.
          </p>
        </div>

        <div className="lab-filter-row reveal" role="group" aria-label="Filter lab entries by status">
          {([
            ['all', 'All entries', counts.all],
            ...
            <button key={key} className={`lab-filter-btn ${filter === key ? 'lab-filter-btn--active' : ''}`}
              onClick={() => setFilter(key)} aria-pressed={filter === key}>
              {key === 'stuck' && <AlertTriangle size={12} aria-hidden="true" />}
              {label} <span className="lab-filter-count">{count}</span>
            </button>
          ))}
        </div>
        ...
        <div className="lab-grid">
          {filtered.map((entry, i) => (
            <LabEntryCard key={entry.id} entry={entry} index={i}
              isOpen={openIds.has(entry.id)} onToggle={() => toggle(entry.id)} />
          ))}
        </div>

        <div className="lab-footer-note reveal">
          <ThumbsUp size={14} aria-hidden="true" />
          <span>
            Spot something I got wrong, or solved a similar problem? I read every message —
            <a href="#contact"> say hello below</a>.
          </span>
        </div>
      </div>
    </section>
...
function Header({ theme, setTheme, onOpenCmd }: { theme: Theme; setTheme: (t: Theme) => void; onOpenCmd: () => void }) {
  ...
  useEffect(() => {
    const ids = ['home','about','skills','experience','projects','lab','certifications','achievements','contact'];
    const onScroll = () => {
      const winH = document.documentElement.scrollHeight - window.innerHeight;
      setScrollPct(winH > 0 ? (window.scrollY / winH) * 100 : 0);
      setVisible(window.scrollY > window.innerHeight * 0.85);
  ...
  const navIds = ['about','skills','projects','lab','contact'];
      ...
      {mobileOpen && (
        <>
          ...
          <nav className="mobile-nav-drawer" aria-label="Mobile navigation">
            ...
            {navIds.map(id => (
              <button key={id} onClick={() => scrollTo(id)}
                className={`mobile-nav-btn ${activeId === id ? 'mobile-nav-btn--active' : ''}`}>
                {id}
              </button>
            ))}
            <button className="mobile-nav-close" onClick={() => setMobileOpen(false)}>✕ close</button>
          </nav>
        </>
      )}
    </>
                  ...
                  <div className="project-card-inner">
                    <div className="project-top-row">
                      ...
                      <div className="project-top-right">
                        {p.isTeam && (
                          <span className="project-team-badge" title="Team project — I was a core contributor">
                            <GitMerge size={10} aria-hidden="true" /> Contributor
                          </span>
                        )}
                        <span className="project-num" aria-hidden="true">0{i + 1}</span>
                      </div>
